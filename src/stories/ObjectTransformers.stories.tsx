...
export const TransformField: Story = {
  args: {
    ...
    description: 'Transform a specific field in an object while preserving all other fields.',
...
export const PickFields: Story = {
  args: {
    ...
    description: 'Extract only the specified fields from an object, creating a new object with just those fields.',
...
export const OmitFields: Story = {
  args: {
    ...
    description: 'Remove specified fields from an object, returning a new object without those fields.',
...
export const RenameFields: Story = {
  args: {
    transformer: (obj: any, fieldMap: Record<string, string>) => renameFields(fieldMap)(obj),
    defaultInput: sampleUser,
    additionalInput: { 'name': 'displayName', 'email': 'emailAddress' },
    description: 'Rename object fields according to a mapping, useful for API response normalization.',
...
export const FlattenObject: Story = {
  args: {
    ...
    defaultInput: sampleUser,
    description: 'Flatten nested objects into dot-notation key-value pairs for easier processing and indexing.',
...
export const MergeObjects: Story = {
  args: {
    transformer: (obj: any, params: any) => {
      ...
      return mergeObjects(override, ...(Array.isArray(sources) ? sources : [sources]))(obj);
    },
    defaultInput: sampleUser,
    additionalInput: { 
      override: true, 
    ...
    description: 'Merge multiple objects with control over whether to override existing values or preserve them.',
...
export const DeepClone: Story = {
  args: {
    ...
    defaultInput: sampleUser,
    description: 'Create a deep copy of an object, ensuring complete independence from the original.',
...
export const KeysToCamelCase: Story = {
  args: {
    ...
    description: 'Convert all object keys from snake_case or other formats to camelCase for JavaScript conventions.',
...
export const KeysToSnakeCase: Story = {
  args: {
    ...
    description: 'Convert all object keys from camelCase to snake_case for API requests and database operations.',
...
const ApiTransformationDemo = ({
  ...
  const [direction, setDirection] = useState<'api-to-ui' | 'ui-to-api'>('api-to-ui');
  ...
  const transformApiToUi = (apiData: any) => {
    ...
    if (transformed.createdAt) {
      transformed.createdDate = formatDateHuman()(new Date(transformed.createdAt));
      delete transformed.createdAt;
    }
    if (transformed.updatedAt) {
      transformed.lastUpdated = formatDateHuman()(new Date(transformed.updatedAt));
    ...
    if (transformed.profileData) {
      ...
      transformed = {
        ...
        jobTitle: profile.jobTitle?.replace(/\b\w/g, (l: string) => l.toUpperCase()),
        department: profile.departmentName?.replace(/\b\w/g, (l: string) => l.toUpperCase()),
        ...
        fullAddress: profile.addressInfo ? 
          `${profile.addressInfo.streetAddress?.replace(/\b\w/g, (l: string) => l.toUpperCase())}, ${profile.addressInfo.cityName?.replace(/\b\w/g, (l: string) => l.toUpperCase())}, ${profile.addressInfo.stateCode} ${profile.addressInfo.zipCode}` : ''
      };
    ...
    if (transformed.accountSettings) {
      ...
      transformed = {
        ...
        lastLoginDisplay: settings.lastLoginAt ? 
          Math.floor((Date.now() - new Date(settings.lastLoginAt).getTime()) / (1000 * 60 * 60 * 24)) + ' days ago' : 'Never'
      };
    ...
    transformed = pickFields([
      'userId', 'firstName', 'lastName', 'displayName', 'emailAddress', 'phoneNumber',
      'createdDate', 'lastUpdated', 'jobTitle', 'department', 'formattedSalary', 
      'startDate', 'isManager', 'skillsCount', 'topSkills', 'fullAddress',
      'themePreference', 'emailNotifications', 'twoFactorEnabled', 'lastLoginDisplay'
    ] as any)(transformed);
  ...
  const transformUiToApi = (uiData: any) => {
    ...
    transformed = omitFields([
      'display_name', 'created_date', 'last_updated', 'formatted_salary',
      'start_date', 'skills_count', 'top_skills', 'full_address', 'last_login_display'
    ] as any)(transformed);
  ...
  const transformer = direction === 'api-to-ui' ? transformApiToUi : transformUiToApi;
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ marginBottom: '10px' }}>
          <strong>Input ({direction === 'api-to-ui' ? 'API Response' : 'UI Form Data'}):</strong>
          <pre
            style={{
              marginTop: '5px',
              padding: '10px',
        ...
        <div style={{ marginBottom: '10px' }}>
          <strong>Output ({direction === 'api-to-ui' ? 'UI Data' : 'API Payload'}):</strong>
          <pre
            style={{
              marginTop: '5px',
              padding: '10px',
